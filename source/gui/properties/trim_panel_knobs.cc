...
void trim_panel::setup_knobs( QJsonArray _knobs, QVBoxLayout *layout, QList<QWidget *> *viewers_gl )
{
    ...
    for ( int i = 0; i < _knobs.count(); i++ )
    {
        ...
        if ( type == "color" )
        {
            ...
            knob_color *_knob_color = new knob_color( knob_object.value( "minimum" ).toDouble(),
        ...
        else if ( type == "integer" )
        {
            ...
            knob_integer *_knob_integer =
                new knob_integer( knob_object.value( "minimum" ).toInt(),
                                  knob_object.value( "maximum" ).toInt(), value );
