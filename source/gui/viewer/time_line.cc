...

{
    ...
    fit_to_range_action->connect_to(this, [this]() { fit_to_range(first_frame, last_frame); });

    action *next_frame_action = new action("Next Frame", "right");
    next_frame_action->connect_to(parent, [this]() { next_frame(); });

    action *previous_frame_action = new action("Previous Frame", "left");
    previous_frame_action->connect_to(parent, [this]() { previous_frame(); });
