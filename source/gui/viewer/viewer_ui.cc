...
void viewer::create_menu()
{
    ...
    overlay_action->connect_to(
        this, [=]() { _viewer_gl->set_overlay( overlay_action->is_checked() ); } );
...
QWidget *viewer::player_setup_ui()
{
    ...
    action *previous_key_frame_action =
        new action( "Previous key frame", "", "previous_key_frame" );
    ...
    //
    range_way_menu =
        new combo_box( {{"Global", "global"}, {"Input", "input"}, {"In/Out", "inout"}} );
