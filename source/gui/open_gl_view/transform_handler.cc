...
void gl_view::tf_handler_draw()
{
    ...
    for ( auto &handler : tf_handlers )
    {
        ...
        handler.x_handler = {get_coordsf( x1 + translate_viewport ),
                             get_coordsf( x2 + translate_viewport )};

        handler.y_handler = {get_coordsf( y1 + translate_viewport ),
        ...
        handler.rotate_handler = {handler.x_handler.p2(),
...
void gl_view::tf_handler_translate_axis( QPoint cursor_position, tf_handler_struct &handler,
                                         bool x_axis )
{
    QPointF cursor = get_coordsf( cursor_position );
    QPointF cursor_click = get_coordsf( click_position );
...
void gl_view::tf_handler_scale_axis( QPoint cursor_position, tf_handler_struct &handler,
                                     bool x_axis )
{
    QPointF translate = get_position( handler.translate );
